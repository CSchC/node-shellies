...
class Device extends EventEmitter {
  static create(type, id, host, mode = null) {
...
class Shelly2 extends Switch {
  constructor(type, id, host) {
    ...
    this._defineProperty('powerMeter0', 111, 0, Number)
    this._defineProperty('relay0', 112, false, Boolean)
    this._defineProperty('rollerPosition0', 113, 0, Number)
    if (type === 'SHSW-25') {
...
class ShellyRGBW2 extends Device {
  constructor(type, id, host, mode = 'color') {
    ...
    this._defineProperty('colorSwitch', 161, false, Boolean, 'color')
    ...
    this._defineProperty('whiteSwitch0', 151, false, Boolean, 'white')
    this._defineProperty('whiteSwitch1', 161, false, Boolean, 'white')
    this._defineProperty('whiteSwitch2', 171, false, Boolean, 'white')
    this._defineProperty('whiteSwitch3', 181, false, Boolean, 'white')
  }
  ...
  async setColor(opts) {
    ...
    if (query.hasOwnProperty('colorSwitch')) {
      query.turn = query.colorSwitch ? 'on' : 'off'
      delete query.colorSwitch
    }
  ...
  async setWhiteChannel(index, brightness, on) {
    await request
